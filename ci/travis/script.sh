...
if [ "${CB_BUILD_AGENT}" == 'python2-linux-x86_64-release' ]; then
     ...
     python2 ./mk_wheel.py --no-emit-status -T -j 1 -DCUDA_ROOT=/usr/local/cuda-9.2 ;
...
if [ "${CB_BUILD_AGENT}" == 'python34-linux-x86_64-release' ]; then
     ...
     install_cuda_linux;
     cd catboost/python-package;
     python3 ./mk_wheel.py --no-emit-status -T -j 1 -DCUDA_ROOT=/usr/local/cuda-9.2 -DPYTHON_CONFIG=/home/travis/virtualenv/python3.4.6/bin/python3-config;
     python ../../ci/webdav_upload.py *.whl
fi
...
if [ "${CB_BUILD_AGENT}" == 'python35-linux-x86_64-release' ]; then
     ...
     install_cuda_linux;
     cd catboost/python-package;
     python3 ./mk_wheel.py --no-emit-status -T -j 1 -DCUDA_ROOT=/usr/local/cuda-9.2 -DPYTHON_CONFIG=/home/travis/virtualenv/python3.5.5/bin/python3-config;
     python ../../ci/webdav_upload.py *.whl
fi
...
if [ "${CB_BUILD_AGENT}" == 'python36-linux-x86_64-release' ]; then
     ...
     install_cuda_linux;
     cd catboost/python-package;
     python3 ./mk_wheel.py --no-emit-status -T -j 1 -DCUDA_ROOT=/usr/local/cuda-9.2 -DPYTHON_CONFIG=/home/travis/virtualenv/python3.6.3/bin/python3-config;
     python ../../ci/webdav_upload.py *.whl
fi
